...
- **Los roles no categorizan servicios ni eventuales por área**: ni `Servicio` ni `Eventual` tienen un campo que distinga "espacios verdes" de "limpieza" en `schema.prisma` — la separación es solo nominal, por el nombre/label del rol asignado como supervisor. Cualquier rol de `ROLES_PEDIDO_TITULAR` puede ser supervisor de cualquier eventual sin importar su naturaleza; lo más cercano a una categorización es el catálogo `TIPOS_TRABAJO_VALIDOS` de `eventuales.service.js` (mezcla tipos de espacios verdes como `PODA_*`/`DESMALEZADO`/`DESMONTE` con `LIMPIEZA_INTEGRAL`), que es una lista libre por eventual, no una propiedad estructural del registro.
