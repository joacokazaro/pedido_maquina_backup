...
export default function TallerHistorialVehiculos() {
  ...
  async function load() {
    try {
      setLoading(true);
      setError("");
      const res = await fetch(`${API_BASE}/admin/taller/vehiculos/historial?limit=300`, { headers: actorHeaders });
      const data = await res.json().catch(() => []);
      if (!res.ok) throw new Error(data?.error || "No se pudo cargar historial de vehiculos");
  ...
  const filtrado = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      data = data.filter((item) =>
        [item.vehiculo?.id, item.vehiculo?.vehiculo, item.vehiculo?.patente, item.usuario?.username, item.usuario?.nombre, item.observacion]
          .filter(Boolean)
  ...
  if (loading) return <div className="p-4">Cargando historial de vehiculos...</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 pb-24">
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <Link to="/admin/taller/movimientos/vehiculos" className="inline-flex items-center rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white">
          Ir a movimientos vehiculos
        </Link>
      </div>

      <div className="mb-4">
        <h1 className="text-2xl font-bold">Historial - Vehiculos</h1>
        <p className="text-sm text-gray-600">Ultimos movimientos registrados de vehiculos en taller.</p>
      </div>
