...
export default function TallerMovimientosVehiculos() {
  ...
  async function load() {
    try {
      setLoading(true);
      setError("");
      const res = await fetch(`${API_BASE}/admin/vehiculos`, { headers: actorHeaders });
      const data = await res.json().catch(() => []);
      if (!res.ok) throw new Error(data?.error || "No se pudieron cargar vehiculos");
      setVehiculos(Array.isArray(data) ? data : []);
    } catch (e) {
      console.error(e);
      setError(e.message || "Error cargando vehiculos");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function ejecutarMovimiento(accion) {
    if (!canEdit) return;
    if (!selected.length) {
      setError("Selecciona al menos un vehiculo");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/taller/vehiculos/movimientos`, {
        method: "POST",
        headers: {
  ...
  if (loading) return <div className="p-4">Cargando movimientos de vehiculos...</div>;
    ...
    <div className="min-h-screen bg-gray-100 px-4 py-8 pb-24">
      <div className="mx-auto max-w-6xl">
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <Link to="/admin/taller/ver/vehiculos" className="inline-flex items-center rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">
            Ver taller vehiculos
          </Link>
        </div>

        <header className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <h1 className="text-2xl font-bold text-gray-800">Registrar Ingreso / Egreso - Vehiculos</h1>
        ...
        {filtrados.map((item) => (
          ...
          {!filtrados.length ? <div className="rounded-xl border border-gray-200 bg-white p-3 text-sm text-gray-500">No hay vehiculos para el filtro aplicado.</div> : null}
        </div>
      </div>
      ...
      <ConfirmModal
        ...
        message={`Se va a registrar ${pendingAccion || "movimiento"} para ${selected.length} vehiculo(s).`}
